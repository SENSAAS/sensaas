...
char s[MAXLINE+1];
 ...
 /* printf(" NATO = %d\n",NATO);*/
  ...
  for (i=0;i<NATO;i++)
      {
fscanf(in_str,"%lf %lf %lf %lf %*s\n",&V3[i][0],&V3[i][1],&V3[i][2],&R[i]);
	XYZ[j]=V3[i][0];
	XYZ[j+1]=V3[i][1];
	XYZ[j+2]=V3[i][2];
	j=j+3;
...
if (NSC(XYZ, R, NATO, density,FLAG_ATOM_AREA | FLAG_VOLUME | FLAG_DOTS,&area,
 &at_area, &volume, &dots, &ndots)) ERROR("error in NSC");


/*
 NSC(co, ra, NAT, 42,FLAG_VOLUME | FLAG_ATOM_AREA | FLAG_DOTS, &area,
      &at_area, &volume,
      &dots, &ndots)
*/
  ...
  for (i=1; i<=NATO; i++) {
    fprintf(fp, "%4d ATOM %7.2f %7.2f %7.2f  R=%4.3f  area=%8.3f\n",
      i, XYZ[3*i-3], XYZ[3*i-2], XYZ[3*i-1], R[i-1], at_area[i-1]);
    }
  fprintf(fp, "\n");
  fprintf(fp, "DOTS : %8d\n", ndots);
  for (i=1; i<=ndots; i++) {
